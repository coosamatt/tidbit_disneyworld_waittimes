...
# Endless loop to push WDW rides to the Tidbyt device.
# This script will reshuffle the list every "round" and automatically
# continue if there are network failures or rendering errors.
#
# Requirements:
# - pixlet installed and on PATH
...
echo "Starting endless loop. Press Ctrl+C to stop."

while true; do
  # Randomize the order for each rotation (round)
  echo "[$(date +%H:%M:%S)] Reshuffling all rides for a new round..."
  
  # Use python to shuffle the array
  IFS=$'\n' SHUFFLED_RIDES=($(printf "%s\n" "${THE_RIDES[@]}" | python3 -c "import random, sys; l = sys.stdin.readlines(); random.shuffle(l); print(''.join(l), end='')"))
  unset IFS

  for entry in "${SHUFFLED_RIDES[@]}"; do
    ...
    echo "[$(date +%H:%M:%S)] Processing ${ride_name} (Park ${park_id}, Ride ${ride_id})"
    
    # Render WEBP - retry indefinitely on failure (e.g. network outage fetching data)
    until "$PIXLET_BIN" render tron_wait.star park_id="${park_id}" ride_id="${ride_id}" --output "${out}"; do
      echo "  [ERROR] Render failed for ${ride_name}. Retrying in 10s..."
      sleep 10
    done
    
    # Push to device - retry indefinitely on failure
    echo "  Pushing to device ${PUSH_DEVICE}..."
    until "$PIXLET_BIN" push "${PUSH_DEVICE}" "${out}" --api-token "${PUSH_TOKEN}"; do
      echo "  [ERROR] Push failed for ${ride_name}. Retrying in 10s..."
      sleep 10
    done
    
    # Successful push delay
    echo "  Waiting 7.5 seconds..."
    sleep 7.5
  done
  
  echo "[$(date +%H:%M:%S)] Finished full round. Restarting..."
done
